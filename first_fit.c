...
//Retorna um ponteiro para o bloco anterior a um bloco disponível
struct Bloco_memoria *scan_first_fit(struct Bloco_memoria*, unsigned int);
...
struct Bloco_memoria *scan_first_fit(struct Bloco_memoria *p, unsigned int tam_req)
{
...
{
    struct Bloco_memoria *x = scan_first_fit(p, tamanho);
